  ...
  return (
    <div className="relative w-[96px] h-[96px] flex items-center justify-center">
      <svg width="96" height="96" className="-rotate-90">
        <circle cx="48" cy="48" r={radius} fill="none" stroke="#fef2f4" strokeWidth="6" />
        <circle
          cx="48"
          cy="48"
          r={radius}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-[22px] font-extrabold text-[#0E0E10] font-allura leading-none">{score}</span>
        <span className="text-[9px] font-bold text-[#6B6B72] uppercase tracking-wider mt-0.5">match</span>
      </div>
    </div>
  );
}
...
export default function VendorProjectDetailPage() {
    ...
    return (
      <div className="min-h-[80dvh] bg-gradient-to-b from-[#fef2f4] to-white flex items-center justify-center px-6">
        <div className="text-center max-w-md">
          <div className="inline-flex items-center justify-center h-16 w-16 rounded-full bg-[#fef2f4] mb-6">
            <ArrowLeft size={28} className="text-[#0E0E10]" />
          </div>
          <h1 className="font-allura text-2xl font-normal text-[#0E0E10] mb-2">Projet introuvable</h1>
          <p className="text-[#6B6B72] mb-6">{error || "Cette opportunité n'est plus disponible."}</p>
          <button
            onClick={() => router.push("/espace-prestataire/appels-offres")}
            className="inline-flex items-center gap-2 h-12 px-6 rounded-full bg-[#e64a5d] text-white hover:brightness-110 font-bold text-sm hover:bg-[#6B6B72] transition"
          >
  ...
  if (isFree || !project) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#fef2f4] to-white font-sans">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
          <button
            onClick={() => router.push("/espace-prestataire/appels-offres")}
            className="inline-flex items-center gap-2 font-semibold text-[10px] uppercase tracking-[0.12em] text-[#6B6B72] hover:text-[#0E0E10] mb-6 transition"
          >
          ...
          >
            ...
            <div className="absolute top-5 right-5 flex items-center gap-2">
              <div className="inline-flex items-center gap-1.5 rounded-full px-3.5 py-2 bg-white/95 backdrop-blur-sm shadow-lg">
                <TrendingUp size={14} className="text-[#0E0E10]" />
                <span className="text-[13px] font-extrabold text-[#0E0E10]">{match.score}%</span>
                <span className="text-[10px] font-bold text-[#6B6B72] uppercase tracking-wider">match</span>
              </div>
            </div>
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8">
              <h1 className="font-allura text-2xl sm:text-4xl font-normal text-white mb-2 leading-tight">
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#fef2f4] to-white font-sans">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-10">
        ...
        <div className="flex items-center justify-between gap-3 px-1 mt-5 mb-6">
          <div className="flex items-center gap-2">
            {isContacted ? (
              ...
              <span className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 bg-[#fef2f4]/30 border border-[#fef2f4] text-[#0E0E10] text-[11px] sm:text-[12px] font-bold">
          ...
          {!isContacted && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full bg-[#e64a5d] text-white hover:brightness-110 px-4 py-2 text-[12px] sm:text-[13px] font-bold hover:bg-[#6B6B72] transition"
            >
        ...
        {matchReasons.length > 0 && (
          <div className="mb-8">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="font-allura text-sm font-normal text-[#0E0E10] uppercase tracking-wider">Pourquoi vous</h2>
            </div>
            <div className="flex flex-wrap gap-2.5">
              {matchReasons.map((reason, i) => (
                <div
                  key={i}
                  className="inline-flex items-center gap-2 rounded-[28px] px-4 py-2.5 bg-white border border-[#EDEDF0] shadow-sm"
                >
                  <div className="w-5 h-5 rounded-full bg-[#fef2f4] flex items-center justify-center shrink-0">
                    <Check size={11} className="text-[#0E0E10]" />
                  </div>
                  <span className="text-[13px] font-medium text-[#0E0E10]">{reason}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-6 lg:gap-8 items-start">
          ...
          <div className="space-y-5">
            ...
            <InfoCard
              ...
              num="02"
            >
              <p className="text-sm text-[#0E0E10]/70 mb-3">
                Ce que les mariés cherchent pour <span className="text-[#0E0E10] font-medium">{match.category}</span>
              </p>
              <p className="text-[15px] text-[#0E0E10]/90 leading-relaxed">
            ...
            {summary?.coupleStory && (
              <InfoCard
                ...
                num="03"
              >
                <p className="text-[15px] text-[#0E0E10]/90 leading-relaxed italic">{summary.coupleStory}</p>
              </InfoCard>
            )}
            ...
            {isContacted && (
              <div className="rounded-[28px] bg-[#e4f4ed] border border-[#b8ddd0] px-6 py-4 flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-[14px] font-bold text-[#0E0E10] mb-0.5">Vous avez déjà répondu</h4>
          ...
          <div className="hidden lg:block lg:sticky lg:top-6 space-y-5">
            {/* Score card */}
            <div className="rounded-[28px] bg-white border border-[#EDEDF0] shadow-[0_10px_30px_rgba(0,0,0,0.06)] p-6 flex flex-col items-center text-center">
              <ScoreRing score={match.score} />
              <p className="text-[12px] font-bold text-[#6B6B72] uppercase tracking-wider mt-3 mb-1">Compatibilité</p>
              <p className="text-[13px] text-[#0E0E10]/80 leading-relaxed">
          ...
          <DialogContent className="sm:max-w-lg rounded-[28px]">
            <DialogHeader>
              <div className="flex items-center gap-3 mb-2">
                <div className="w-12 h-12 rounded-[28px] bg-[#fef2f4] flex items-center justify-center">
                  <Send size={22} className="text-[#0E0E10]" />
                </div>
                <div>
                  <p className="text-[#6B6B72] text-xs font-bold uppercase tracking-wider">Appel d'offres</p>
                  <DialogTitle className="font-allura text-xl font-bold text-[#0E0E10]">
                    Répondre au couple
                  </DialogTitle>
                </div>
              </div>
              <DialogDescription className="text-[#6B6B72]">
                Rédigez un message personnalisé pour ce couple.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 mt-2">
              <div>
                <label className="block text-sm font-medium text-[#0E0E10] mb-2">Message personnalisé</label>
                <textarea
                  value={message}
                  onChange={(e) => setMessage(e.target.value)}
                  rows={5}
                  placeholder="Bonjour, je suis intéressé par votre projet..."
                  className="w-full rounded-[28px] border-2 border-[#EDEDF0] bg-white px-4 py-3.5 text-[#0E0E10] focus:outline-none focus:border-[#E4DBFB] transition resize-none"
                />
              </div>
              <button
                onClick={respond}
                disabled={submitting || !message.trim()}
                className="w-full inline-flex items-center justify-center gap-2 h-12 rounded-full bg-[#e64a5d] text-white hover:brightness-110 font-bold text-sm hover:bg-[#6B6B72] transition disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        <Dialog open={success} onOpenChange={() => setSuccess(false)}>
          <DialogContent className="sm:max-w-md text-center rounded-[28px]">
            <DialogHeader>
              <div className="h-16 w-16 rounded-full mx-auto mb-4 flex items-center justify-center bg-[#fef2f4]">
                <CheckCircle2 size={28} className="text-[#0E0E10]" />
              </div>
              <DialogTitle className="font-allura text-xl font-bold text-[#0E0E10]">
                Proposition envoyée
              </DialogTitle>
            ...
            <div className="flex flex-col gap-3 mt-4">
              <button
                ...
                className="w-full h-12 rounded-full bg-[#e64a5d] text-white hover:brightness-110 font-bold text-sm hover:bg-[#6B6B72] transition"
              >
                Continuer
              </button>
              <button
                onClick={() => router.push("/espace-prestataire/propositions")}
                className="w-full h-12 rounded-full border-2 border-[#EDEDF0] bg-white text-[#0E0E10] font-bold text-sm hover:bg-[#fef2f4] transition"
              >
...
function FicheTechnique({
  ...
  return (
    <div className="rounded-[28px] bg-white border border-[#EDEDF0] shadow-[0_10px_30px_rgba(0,0,0,0.06)] overflow-hidden">
      <div className="bg-[#0E0E10] px-5 py-3.5">
        <div className="font-allura text-[13px] font-bold text-white uppercase tracking-wider">
          Fiche technique
        </div>
      </div>
...
function InfoCard({
  ...
  return (
    <div className={`rounded-[28px] ${bg} p-6 relative overflow-hidden`}>
      <span className="absolute top-4 right-5 font-allura text-[40px] font-bold text-[#0E0E10]/8 leading-none select-none">{num}</span>
      <div className="flex items-center gap-3 mb-4 relative z-10">
        <div className="h-9 w-9 rounded-full bg-[#0E0E10]/10 flex items-center justify-center text-[#0E0E10]">
          {icon}
        </div>
        <span className="font-sans text-[13px] text-[#0E0E10]/80 font-bold uppercase tracking-wider">{label}</span>
      </div>
...
function SpecRow({
  ...
  return (
    <div className="flex items-end gap-2">
      <span className="inline-flex items-center gap-1.5 text-xs text-[#6B6B72] shrink-0">
        {icon && <span className="text-[#0E0E10]">{icon}</span>}
        {label}
      </span>
      <span className="flex-1 border-b border-dotted border-[#EDEDF0] mb-1" aria-hidden />
      <span className="text-sm font-medium text-[#0E0E10] shrink-0">{value}</span>
    </div>
  );
}