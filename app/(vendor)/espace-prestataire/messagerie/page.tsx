  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-6 lg:py-10 h-[calc(100dvh-4rem)]">
      <PageHeader
        label="Messages"
        title="Messagerie"
        subtitle="Échangez avec les couples et finalisez vos prestations."
      />
      ...
      {proposals.length === 0 ? (
        ...
        <div className="grid lg:grid-cols-[360px_1fr] gap-5 h-[calc(100%-7rem)] min-h-[500px]">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Avatar name={p.project?.name || "M"} className="h-10 w-10 text-sm shrink-0" />
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="font-medium text-text-primary truncate text-sm">{p.project?.name || "Mariage"}</span>
            ...
            {selected ? (
              <>
                ...
                <div className="p-4 border-b border-black/[0.06] flex items-center gap-3">
                  ...
                  <Avatar name={selected.project?.name || "M"} className="h-11 w-11 text-base" />
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-text-primary truncate">{selected.project?.name || "Mariage"}</div>
                    <div className="flex items-center gap-3 text-xs text-text-secondary">
                      {selected.project?.location?.city && <span className="flex items-center gap-1"><MapPin size={11} /> {selected.project.location.city}</span>}
                      {selected.project?.weddingDate && <span className="flex items-center gap-1"><Calendar size={11} /> {new Date(selected.project.weddingDate).toLocaleDateString("fr-FR")}</span>}
                    </div>
                  </div>
