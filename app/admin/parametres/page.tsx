    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleCreate} className="bg-white rounded-[20px] border border-[#f1f5f9] p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <input required type="number" min={1} step={1} onKeyDown={(e) => { if (e.key === "-" || e.key === "e") e.preventDefault(); }} value={form.price} onChange={(e) => setForm(f => ({ ...f, price: Math.max(1, Number(e.target.value) || 1) }))} placeholder="Prix (€)" className="rounded-lg border border-[#f1f5f9] px-3 py-2.5 text-sm text-[#1e293b] focus:outline-none focus:ring-2 focus:ring-[#db2777]/20" />
          <input required type="number" min={1} step={1} onKeyDown={(e) => { if (e.key === "-" || e.key === "e") e.preventDefault(); }} value={form.commitmentMonths} onChange={(e) => setForm(f => ({ ...f, commitmentMonths: Math.max(1, Number(e.target.value) || 1) }))} placeholder="Mois d'engagement" className="rounded-lg border border-[#f1f5f9] px-3 py-2.5 text-sm text-[#1e293b] focus:outline-none focus:ring-2 focus:ring-[#db2777]/20" />
        </div>
